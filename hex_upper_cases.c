#include "ft_printf_bonus.h"
	...
	void hexnumber_upper_case_one(const char *str, t_format *flg, int w_nbr, int len, int *count)
	{
		*count = 1;
		...
		if(flg->minus_flag && flg->hash_flag || flg->zero_flag && flg->hash_flag)
			combined_flags_hex_upper_case_one(str,flg,w_nbr,len,count);
		else if(flg->minus_flag||flg->hash_flag)
			single_flags_hex_upper_case_one(str,flg,w_nbr,len,count);
		else
		{
			hex_case_check_upper_padding(flg, w_nbr, count);
			printing_string_upper_hex(flg, len, str, w_nbr, count);
		}
	}
    ...
    void hexnumber_upper_case_three(const char *str, t_format *flg, int p_nbr, int w_nbr,int len, int *count)
	{
		...
		if(flg->hash_flag && flg->minus_flag || flg->hash_flag)
			hex_upper_cases_in_case_three(str,flg,w_nbr,p_nbr,len,count);
